...
static void Modify_Direction(uint16_t difference);
...
void MotorControl_Init(void)
{
	...
	g_base_motor.deg_per_turn = 0.9f;
...
{
	uint16_t difference = g_base_motor.motor_angle_to_reach - g_base_motor.current_motor_angle;
	if (difference != 0)
	{
...
void Modify_Speed(void)
{
	...
	uint16_t new_freq = g_base_motor.motor_speed_percent * FREQ_MAX_HZ;
	...
	g_base_motor.nb_pulse = (g_base_motor.motor_angle_to_reach - g_base_motor.current_motor_angle) / g_base_motor.deg_per_turn ;
...
void Modify_Direction(uint16_t difference)
{
